import 'package:switrans_2_0/src/packages/maestro/resolucion/data/resolucion_data.dart';
import 'package:switrans_2_0/src/packages/maestro/resolucion/domain/resolucion_domain.dart';
import 'package:switrans_2_0/src/util/resources/base_api.dart';
import 'package:switrans_2_0/src/util/resources/data_state.dart';

class ResolucionRespositoryDBImpl extends BaseApiRepository implements AbstractResolucionRepository {
  final ResolucionDB _resolucionDB;

  ResolucionRespositoryDBImpl(this._resolucionDB);

  @override
  Future<DataState<List<ResolucionDocumento>>> getDocumentosService() async {
    final DataState<dynamic> httpResponse = await getStateOf(request: () => _resolucionDB.getDocumentosDB());
    if (httpResponse.data != null) {
      final dynamic resp = httpResponse.data['data'];
      final List<ResolucionDocumento> response =
          List<ResolucionDocumento>.from(resp.map((dynamic x) => ResolucionDocumentoModel.fromDB(x)));
      return DataSuccess<List<ResolucionDocumento>>(response);
    }
    return DataFailed<List<ResolucionDocumento>>(httpResponse.error!);
  }

  @override
  Future<DataState<List<ResolucionEmpresa>>> getEmpresasService() async {
    final DataState<dynamic> httpResponse = await getStateOf(request: () => _resolucionDB.getEmpresasDB());
    if (httpResponse.data != null) {
      final dynamic resp = httpResponse.data['data'];
      final List<ResolucionEmpresa> response = List<ResolucionEmpresa>.from(resp.map((dynamic x) => ResolucionEmpresaModel.fromDB(x)));
      return DataSuccess<List<ResolucionEmpresa>>(response);
    }
    return DataFailed<List<ResolucionEmpresa>>(httpResponse.error!);
  }

  @override
  Future<DataState<List<Resolucion>>> getResolucionesService(ResolucionRequest request) async {
    final DataState<dynamic> httpResponse = await getStateOf(request: () => _resolucionDB.getResolucionesDB(request));
    if (httpResponse.data != null) {
      final dynamic resp = httpResponse.data['data'];
      final List<Resolucion> response = List<Resolucion>.from(resp.map((dynamic x) => ResolucionModel.fromDB(x)));
      return DataSuccess<List<Resolucion>>(response);
    }
    return DataFailed<List<Resolucion>>(httpResponse.error!);
  }

  @override
  Future<DataState<Resolucion>> setResolucionService(ResolucionRequest request) async {
    final DataState<dynamic> httpResponse = await getStateOf(request: () => _resolucionDB.setResolucionesDB(request));
    if (httpResponse.data != null) {
      final dynamic dataJson = httpResponse.data['data'];
      final Resolucion response = ResolucionModel.fromDB(dataJson);
      return DataSuccess<Resolucion>(response);
    }
    return DataFailed<Resolucion>(httpResponse.error!);
  }

  @override
  Future<DataState<Resolucion>> updateResolucionService(ResolucionRequest request) async {
    final DataState<dynamic> httpResponse = await getStateOf(request: () => _resolucionDB.updateResolucionesDB(request));
    if (httpResponse.data != null) {
      final dynamic resp = httpResponse.data['data'];
      final List<Resolucion> response = List<Resolucion>.from(resp.map((dynamic x) => ResolucionModel.fromDB(x)));
      return DataSuccess<Resolucion>(response.first);
    }
    return DataFailed<Resolucion>(httpResponse.error!);
  }

  @override
  Future<DataState<List<ResolucionCentroCosto>>> getCentroCostoService(ResolucionEmpresa request) async {
    final DataState<dynamic> httpResponse = await getStateOf(request: () => _resolucionDB.getCentroCostoDB(request));
    if (httpResponse.data != null) {
      final dynamic resp = httpResponse.data['data'];
      final List<ResolucionCentroCosto> response =
          List<ResolucionCentroCosto>.from(resp.map((dynamic x) => ResolucionCentroCostoModel.fromDB(x)));
      return DataSuccess<List<ResolucionCentroCosto>>(response);
    }
    return DataFailed<List<ResolucionCentroCosto>>(httpResponse.error!);
  }
}
