...
class FilterPocketBase extends ModuloRequest {
  String toPocketBaseFilter(ModuloRequest request) {
    ...
    if (request.moduloCodigo != null) {
      conditions.add('modulo_codigo = ${request.moduloCodigo!}');
    }
    if (request.paquete != null && request.paquete!.isNotEmpty) {
      conditions.add('paquete = "${request.paquete!}"');
    }
    if (request.moduloVisible != null) {
      conditions.add('modulo_visible = ${request.moduloVisible!}');
    }
    if (request.moduloActivo != null) {
